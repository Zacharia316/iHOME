...
function SignInScreen({ onSignIn, onGoCreate }) {
  ...
  const handle = async () => {
    ...
    const { data: profile, error: profileErr } = await supabase
      ...
      .single();
    setLoading(false);
    if (profileErr || !profile) { setError('Profile not found. Please create an account.'); return; }
    onSignIn({ email, id: data.user.id, username: profile.username, role: profile.role });
  };
